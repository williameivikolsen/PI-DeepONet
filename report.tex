...
scalar flux $\varphi_0(x)$, it is more important for the model to learn this quantity well than the angular flux $\psi(x, \mu)$.
...
half the value of the mean in the training dataset. This corresponds to a lower scalar flux, giving higher relative errors. Likewise, for the rest of the large
shifts, where the mean of the source distributions are higher, the performance appears to be better, likely because of the higher mean scalar flux.
For the single test cases, the performance is worst for the nonlinear combination of two source distributions.

For the models trained on the medium sized dataset, shown in Figures \ref{fig: R2 medium} and \ref{fig: ARE medium}, we see a change in which models
...
\begin{figure*}[t]
    ...
    \caption{Samples of predictions made by PI DeepONet 1 trained on original dataset along with source distribution $Q(x)$ and true scalar flux $\varphi_0(x)$.}
