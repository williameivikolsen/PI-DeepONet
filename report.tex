...
physics-informed models performed better. We conclude that physics-informed training does, to some extent, replace labelled training.
\end{abstract}
...
model predicts the scalar flux directly, but the 1D NTE \eqref{eq: 1D NTE} is formulated in terms of the angular flux $\psi(x, \mu)$. The PDE loss term
$\mathcal{L}_{\text{PDE}}$ measures how far \eqref{eq: 1D NTE} is from being satisfied by the model's predictions. It is calculated by drawing $M$ random
samples of $\tilde{\psi}_{n,j}$ and calculating the residual
\begin{align}
R_j &= \frac{\mu_n}{h_j} \left( \tilde{\psi}_{n, j+\frac{1}{2}} - \tilde{\psi}_{n, j-\frac{1}{2}} \right) + \Sigma_{t,j} \, \tilde{\psi}_{n,j} \nonumber \\
&- \frac{1}{2} \left( \Sigma_{s0,j} \, \tilde{\varphi}_{0,j} + 3 \mu_n \Sigma_{s1,j} \, \tilde{\varphi}_{1,j} + Q_j \right),
\end{align}
where $\tilde{\psi}_{n,j}$, $\tilde{\varphi}_{0,j}$ and $\tilde{\varphi}_{1,j}$ are the model's predictions for the angular flux, scalar flux and scalar current, respectively.
The PDE loss term is then given by
\begin{equation}
\mathcal{L}_{\text{PDE}} = \frac{1}{M} \sum_{j=1}^M R_j^2.
\end{equation}
...
It is calculated by drawing $M$ random samples of $\tilde{\psi}_{n,j}$ at the boundaries $x = 0$, $x=10$, and evaluating how far the model is from predicting
the correct boundary condition $\psi(x=0, \mu) = 0$ for $\mu > 0$ and $\psi(x=10, \mu) = 0$ for $\mu < 0$. The loss term is then given by
\begin{equation}
\mathcal{L}_{\text{BC}} = \frac{1}{M} \sum_{j=1}^M \tilde{\psi}_{n,j}^2.
\end{equation}

To implement this loss funciton, the Physics-Informed DeepONet must therefore predict the angular flux $\psi(x,\mu)$ instead of the scalar flux $\varphi_0(x)$.
...
Figures \ref{fig: R2 large} and \ref{fig: ARE large}. We see that it achieved an $R^2$ score of $0.999 \pm 0.005$ and ARE score of $(1.1 \pm 0.2)\%$ 
...
with the PDE loss contributing negatively to the models' performance when trained on the original dataset.
...
perform well. Unsurpisingly, the general performance is worse than for the models trained on the original dataset. The benchmark model performs significantly
...
better. The Large DeepONet seems to have settled on a set of parameters that gives a similar prediction different distributions – one that gives a less
...
Finally, for the models trained on Dataset 3, we see in Figures \ref{fig: R2 small} amd \ref{fig: ARE small} that the performance has dropped further. The
...
dataset, however, we still see that the ordinary DeepONet performs better. It should be noted that the work in this paper is so far limited to the first
...
\begin{figure*}[t]
    ...
    \caption{Comparison of NS sample predictions made by all models on trained on Dataset 3.}
