...
can generate training data. The core research question of this study is \textit{to which degree physics-informed training can replace labelled
...
data will be more scarce due to the necessity of using Monte Carlo or Deterministic methods for training data generation.
...
For the benchmark model of \cite{sahadath2026characterization}, the model was trained on
\begin{itemize}
    \item Input data: The 500 source distribution samples $Q(x)$ described above.
    \item Output data: The corresponding scalar flux $\varphi(x)$ for each source distribution, given by equation 
\end{itemize}
...
Figure \ref{fig: benchmark sample}. The model was tested by the scenarios described by \cite{sahadath2026characterization}, with the results
shown in Table \ref{tab:deeponet_results}. NOTE: Include Sahadath results in this table.

We observe that our DeepONet model performs similarly to that of Sahadath. The benchmark model has therefore been successfully implemented.
