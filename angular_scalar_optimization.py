...
from model import (
    DataGenerator,
    build_data_arrays, build_bcs_arrays, build_res_arrays, build_val_batch,
)
from model_angular_scalar import PI_DeepONet_AngularScalar

# ---------------------------------------------------------------------------
# Transition steps follow the report's PI schedule: decay every 200 epochs
# (n // 10), against 100 epochs for the benchmark and Large DeepONet.
LR_CANDIDATES = {
    "const_1e-2": lambda n: 1e-2,
    "const_3e-3": lambda n: 3e-3,
    "const_1e-3": lambda n: 1e-3,      # what angular_scalar_training.py currently uses
...
# PI DeepONet 1: vector-output angular trunk, supervised on scalar phi_0.
# Architecture and loss weights are those of angular_scalar_training.py.
model_name   = "pideeponet_angular_scalar_data"
N_LAYERS     = 4
P_LATENT     = 100
BRANCH_WIDTH = 250
TRUNK_WIDTH  = 500
...
def passes_guard(model, tol=1.0):
    """
    Same save-time check as angular_scalar_training.py: the restored params
    must reproduce best_val_ARE on both the training metric and the
    predict_phi0 evaluation path, and the flux must be non-negative.
    """
